...
export function calculateReadinessLevel(completedSections: number, totalSections: number): ReadinessLevel {
    const percentage = totalSections === 0 ? 0 : (completedSections / totalSections) * 100;
...
export type ContractType = 
    ...
    | 'employment_contract'
    | 'other_contract'
    // New ownership & legal document types
    | 'business_license'
    | 'operating_agreement'
...
export interface DigitalAsset {
    ...
    user_id: string;
    name: string;
    type: 'domain' | 'hosting' | 'email' | 'social_media' | 'website' | 'saas' | 'other';
    provider: string;
...
export interface PhysicalAsset {
    ...
    user_id: string;
    name: string;
    type: 'vehicle' | 'equipment' | 'property' | 'inventory' | 'furniture' | 'other';
...
export interface Subscription {
    ...
    provider: string;
    tier?: string;
    billing_cycle: 'monthly' | 'quarterly' | 'annual' | 'other';
    renewal_date?: string;
    monthly_cost?: number;
    credential_location?: string;
