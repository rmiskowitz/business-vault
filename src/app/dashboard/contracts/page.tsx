      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-1 text-[#71717a]">
            Document contracts, ownership, and financial records
          </p>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#0f0f12] border border-[#1e1e23] rounded-xl p-6 mb-8 space-y-5">
          <h2 className="text-lg font-medium text-white mb-4">
            {editingId ? 'Edit Document' : 'Document New Item'}
          </h2>
              ...
              <optgroup label="Contracts">
                {CONTRACT_TYPE_CATEGORIES.contracts.map((type) => (
                  <option key={type} value={type}>
                    {CONTRACT_TYPE_LABELS[type as ContractType]}
                  </option>
                ))}
              </optgroup>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-[#0f0f12] rounded-xl border border-[#1e1e23]">
              ...
              <p className="text-[#71717a] mb-6">
                Start by documenting your contracts and legal documents.
              </p>
              <button
                onClick={() => setShowForm(true)}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {contract.counterparty && (
                        <p className="text-sm text-[#52525b]">Party: {contract.counterparty}</p>
                      )}
